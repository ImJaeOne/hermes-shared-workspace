...
### 기획 자료조사와 Hermes 기반 자가 발전

기획 자료조사 MVP는 단순 Slack 챗봇이 아니라, AX Dashboard/Slack Adapter/worker/Hermes Agent를 묶어 자료조사와 수정 이력을 점진 개선 후보로 축적하는 업무 자동화 흐름입니다.

- **AX Slack Adapter**는 Slack 이벤트를 AX workflow, source material, worker request/result, artifact, Dashboard 기록으로 연결합니다.
- **Hermes Gateway**는 Slack에서 범용 Hermes Agent와 직접 대화하기 위한 별도 선택 기능이며, AX 자료조사 MVP의 필수 경로는 아닙니다.
- 비개발자 직원은 Codex/OpenAI 인증을 하지 않고 Slack/AX만 사용합니다. 서버는 persistent `HERMES_HOME=/data/.hermes`에 설정된 회사 공용 Hermes provider credential을 사용합니다.
- 일반 사용자 수정 요청은 현재 산출물만 수정합니다. `planninglearner` 또는 learner approval role을 가진 경로에서만 prompt/skill/memory 개선 후보를 만들고, 승인된 후보만 runtime에 적용합니다.

전체 결정과 후속 Issue 로드맵은 `docs/decisions/planning-research-self-improvement.md`를 기준으로 합니다.
...
Hermes Agent를 서버에 함께 배포하는 이유, AX Slack Adapter와 Hermes Gateway의 차이, 일반 사용자와 `planninglearner`의 자가 발전 경계, #39 → #48 → #50 → #42 → #26 후속 로드맵은 `docs/decisions/planning-research-self-improvement.md`를 기준으로 합니다.
